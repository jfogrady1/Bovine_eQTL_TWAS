...
res_no_sva <- function(counts_in, coldata_in) {
    ...
    genotype_PCs <- genotype_PCs %>% select(-1)
    genotype_PCs <- genotype_PCs %>% select(1:5)
    colnames(genotype_PCs) <- paste0("Genotype_PC", 1:5)
    ...
    ddsMat <- DESeqDataSetFromMatrix(countData = counts_in,
                                    colData = coldata,
                                    design = ~ Batch + Genotype_PC1 + Genotype_PC2 + Genotype_PC3 + Genotype_PC4 + Genotype_PC5 + Age + Condition)
